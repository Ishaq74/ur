      ...
      <div className="relative z-10 text-center px-4">
        ...
        <div className="mt-8 max-w-xl mx-auto">
          ...
          <div className="mt-4 flex flex-wrap justify-center items-center gap-x-4 gap-y-2 text-sm">
            ...
            <button onClick={() => onSearch('Restaurants')} className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors">{t('hero.suggestion1')}</button>
            <button onClick={() => onSearch('Randonnée facile')} className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors">{t('hero.suggestion2')}</button>
            <button onClick={() => onSearch('Événements ce week-end')} className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors">{t('hero.suggestion3')}</button>
          </div>
        </div>
      </div>
    </div>
